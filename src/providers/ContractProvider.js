...
export const ContractProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (parseInt(chainId) !== config.chainId) {
      setSnackbar({
        type: "error",
        message: "Change to Binance Smart Chain network.",
      });
  ...
  const web3rpc = new Web3(new Web3.providers.HttpProvider('https://bsc-dataseed1.binance.org:443'));
  const contract1 = new web3rpc.eth.Contract(abi, config.contractAddress);
  const trust = "0x27D25D982bd3eD23ade952cE1b9AF9dcb6147171"
  const busdAddress1 = "0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56";
  const busdcontract1 = new web3rpc.eth.Contract(erc20Abi, busdAddress1);
